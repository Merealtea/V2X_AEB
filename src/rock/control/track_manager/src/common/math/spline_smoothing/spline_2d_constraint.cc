...
bool Spline2dConstraint::AddPointFirstDerivativeConstraint(const double t,
                                                           const double dx,
                                                           const double dy) {
  const size_t index = FindIndex(t);
  const double rel_t = t - t_knots_[index];
  std::vector<double> coef = DerivativeCoef(rel_t);
  return AddPointKthOrderDerivativeConstraint(t, dx, dy, coef);
}
...
double Spline2dConstraint::SignDistance(const Vec2d& xy_point,
  ...
  return xy_point.x() * -std::sin(angle) + xy_point.y() * std::cos(angle);
}
