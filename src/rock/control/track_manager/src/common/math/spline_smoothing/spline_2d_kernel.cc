...
using cyberc3::common::math::Vec2d;
...
bool Spline2dKernel::AddAlignmentKernel(
    const std::vector<double>& last_solution, const double weight) {
  if (last_solution.size() != total_params_) {
    return false;
  }
  Eigen::MatrixXd id_matrix =
      Eigen::MatrixXd::Identity(kernel_matrix_.rows(), kernel_matrix_.cols());
  Eigen::MatrixXd offset = Eigen::MatrixXd::Zero(kernel_matrix_.rows(), 1);
  for (uint32_t i = 0; i < last_solution.size(); ++i) {
    offset(i, 0) = -2.0 * last_solution[i];
  }
  return AddKernel(id_matrix, offset, weight);
}
