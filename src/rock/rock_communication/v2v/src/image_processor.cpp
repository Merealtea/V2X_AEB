...
public:
    ...
    {
        // 获取shm
        std::string shm_name = "rock_image_shm";
        ...
        // 订阅四个压缩图像话题
        sub1_.subscribe(nh_, "/driver/fisheye/left/compressed", 1);
        sub2_.subscribe(nh_, "/driver/fisheye/right/compressed", 1);
        sub3_.subscribe(nh_, "/driver/fisheye/front/compressed", 1);
        sub4_.subscribe(nh_, "/driver/fisheye/back/compressed", 1);
        ...
        // 发布自定义消息
        pub_ = nh_.advertise<hycan_msgs::Localization>("rock_processed_images", 1);
    }
                            ...
                            const hycan_msgs::Localization::ConstPtr& msg5)
    {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            // 1. 先resize
            cv::resize(cv_image, cv_image, cv::Size(new_width, new_height), 0, 0, cv::INTER_AREA);  // 缩放到640x480
            ...
            resized_img[i].stamp = st_time.toSec();//  msgs[i]->header.stamp.toSec(); //st_time.toSec();
