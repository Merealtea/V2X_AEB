...
# --tab --title "vehicle_visual_tracking" -e 'bash -c "source ~/anaconda3/etc/profile.d/conda.sh; conda activate Pytracking; echo $(which python); rosrun components_visual_tracking vehicle_tracking.py; exec bash;"' \
...
--tab --title "vehicle_visual_tracking" -e 'bash -c "source ~/anaconda3/etc/profile.d/conda.sh; conda activate Pytracking; echo $(which python); rosrun components_visual_tracking vehicle_tracking.py; exec bash;"' \
--tab --title "components_tracking" -e 'bash -c "source ~/anaconda3/etc/profile.d/conda.sh; conda activate Pytracking; echo $(which python); roslaunch --wait components_visual_tracking components_tracking.launch; exec bash;"' \
--tab --title "visual_det" -e 'bash -c "source ~/anaconda3/etc/profile.d/conda.sh; conda activate Pytracking; echo $(which python); rosrun components_visual_tracking one_rearlight_tracking.py; exec bash;"' \
--tab --title "tracking" -e 'bash -c "roslaunch --wait ../src/tracking/launch/tracking.launch; exec bash;"' \
--tab --title "tracking_visualization" -e 'bash -c "rviz -d ../src/tracking/rviz/tracking.rviz; exec bash;"' \
